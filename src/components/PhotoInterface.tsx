...
import { Send, Mic, Play, Square, Bot, User, AlertCircle, Globe, ListChecks, CheckCircle2, Circle, Volume2, VolumeX, RotateCcw, PlayCircle, MicOff, Image as ImageIcon, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function PhotoInterface({
    messages,
    onSendMessage,
    isLoading,
    isTtsEnabled,
        ...
        <div className="flex flex-col h-full bg-[#b2c7da] relative font-sans">
            ...
            <div className="px-4 py-3 bg-[#b2c7da] flex items-center justify-between z-10 shadow-sm relative shrink-0">
                ...
                <div className="flex items-center gap-1 opacity-90">
                    <span className="font-bold text-lg text-[#3b1e1e]">Photo Talk</span>
            ...
            {/* Photo Mode Image Card */}
            {photoUrl && (
                <div className="shrink-0 bg-[#E5DDD0] pt-2 pb-2 z-10 px-2">
                    {/* Card Header: progress + toggle button */}
                    {(() => {
                        const lastAiParsed = [...messages].reverse().find(m => m.role === 'assistant' && m.parsed)?.parsed;
                        const count = lastAiParsed?.sentence_count ?? 0;
                        return (
                            <button
                                onClick={() => setIsPhotoExpanded(prev => !prev)}
                                className="w-full flex items-center justify-between px-1 pb-1.5 cursor-pointer group"
                            >
                                <span className="text-xs text-[#3b1e1e]/60 font-medium group-hover:text-[#3b1e1e]/90 transition-colors">문장 연습</span>
                                <div className="flex items-center gap-2">
                                    <div className="flex gap-1.5">
                                        <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${count >= 1 ? 'bg-[#3b1e1e]' : 'bg-[#3b1e1e]/20'}`} />
                                        <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${count >= 2 ? 'bg-[#3b1e1e]' : 'bg-[#3b1e1e]/20'}`} />
                                    </div>
                                    <span className="text-xs font-bold text-[#3b1e1e]/70">{count}/2</span>
                                    <span className="ml-1 text-[#3b1e1e]/50 group-hover:text-[#3b1e1e]/80 transition-colors">
                                        {isPhotoExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                                    </span>
                                </div>
                            </button>
                        );
                    ...
                    {/* Collapsible photo + keyword */}
                    <div
                        className={`overflow-hidden transition-all duration-300 ease-in-out ${isPhotoExpanded ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'
                            }`}
                    >
                        <div className="relative w-full aspect-video bg-gray-900 rounded-2xl overflow-hidden shadow-md border border-white/20">
                            <img
                                src={photoUrl}
                                alt="Topic"
                            ...
                            />
                            {keyword && (
                                <div className="absolute bottom-3 left-1/2 transform -translate-x-1/2 bg-black/60 backdrop-blur-md px-5 py-2 rounded-full border border-white/20 shadow-lg">
                                    <span className="text-white font-bold text-sm tracking-wide">{keyword}</span>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {sessionEnded && (
                <div className="bg-emerald-50 border-t border-emerald-200 px-4 py-3 text-center">
                    <div className="text-sm font-bold text-emerald-700 mb-1">🎉 학습이 완료되었습니다!</div>
                    <div className="text-xs text-emerald-600">제시어 문장 연습을 모두 마쳤습니다. 리셋 버튼을 눌러 새 학습을 시작하세요.</div>
                </div>
            )}
            ...
            <div className={`bg-white p-3 pb-safe-area flex items-center gap-2 border-t border-gray-100 ${sessionEnded ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <button
                    onClick={() => alert('Photo Upload Mock')}
                    className="p-2 text-indigo-500 hover:text-indigo-600 transition-colors bg-indigo-50 rounded-full"
                    title="Upload Photo"
                >
                    <ImageIcon size={20} />
                </button>
